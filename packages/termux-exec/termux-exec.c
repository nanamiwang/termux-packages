...
{
	strcpy(buffer, "/data/com.termux/files/usr/bin/");
	...
	if (bin_match == filename || bin_match == (filename + 4)) {
		...
		strncpy(buffer + 36 - 5, bin_match + 5, buffer_len - (37 - 5));
		filename = buffer;
	}
	return filename;
}
...
final:
	...
	if (android_10_wrapping) {
		...
		if (realpath_ok) {
			bool wrap_in_proot = (strstr(realpath_buffer, "/data/com.termux/files") != NULL);
			if (android_10_debug) {
			...
			if (wrap_in_proot) {
				...
				filename = "/data/com.termux/files/usr/bin/proot";
				new_argv[0] = "proot";
